...
class context:
    ...
    # New: Control EOF behavior on incomplete recvline (like pwntools)
    # None: Warn and return partial data
    # True: Raise EOFError
    # False: Return partial data silently
    ...
    def set_timeout(timeout):
        ...
        if timeout is not None and timeout == "default":
            context.timeout = 10  # Reset to default
    ...
    def _render(self, final=False, success=None):
        ...
        with _log_lock:
            ...
            if final:
                ...
                final_message = f"{level_tag} {self.message}: {self.status_msg}{elapsed}"
                ...
                spinner = context.progress_spinner[context.progress_idx % len(context.progress_spinner)]
                context.progress_idx += 1
                current_line = f"{prefix}{self.status_msg} {spinner}"
                sys.stdout.write(current_line)
                sys.stdout.flush()
                _last_progress_len = len(current_line)  # Track length to clear next time
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if self.active:
            ...
            if exc_type:
                ...
                if self.active:  # Check active again, might have been set False by success/failure
                    self.success()
...
def _format_log_message(message, level, length, is_input, show_hexdump):
    ...
    actual_length_info = f" {display_length} bytes" if display_length is not None else ""
    ...
    if do_hexdump:
        ...
        elif not hex_lines:  # Should not happen if message is bytes unless hexdump fails
        ...
        if level == "debug" and is_bytes and not show_hexdump:
            ...
            lines_to_output.append(first_line_prefix + f" (Hexdump disabled) Raw: {repr(message)}")
        else:
            ...
            if not message_lines:
                ...
                if is_bytes or length is not None:  # Assume it's I/O related if bytes or length given
...
def long_to_bytes(n, length=None, endian=None, signed=False):
    ...
    try:
        ...
        if length is None:
            ...
            if n != 0 and signed and l * 8 < n.bit_length() + 1:  # Need space for sign bit
...
class IOBase:
    ...
    def recvline(self, drop=True, timeout="default") -> bytes:
        """Receives a line, terminated by newline (default \\n)."""
        newline = b'\n' # Hardcode for simplicity, could use context.newline
        try:
             line = self.recvuntil(newline, drop=False, timeout=timeout)
             if drop:
                  # Only strip if newline was actually present
                  if line.endswith(newline):
                       return line[:-len(newline)]
                  else: # Received partial line due to EOF/timeout in recvuntil
                       # Implement new throw_eof_on_incomplete_line behavior
                       if context.throw_eof_on_incomplete_line is True:
                           self.log("EOF received during recvline with incomplete line", "warning")
                           raise EOFError("EOF received during recvline with incomplete line")
                       elif context.throw_eof_on_incomplete_line is None:
                           self.log("Returning partial line due to EOF/timeout", "warning")
                           return line
                       else:  # False - return silently
                           return line
             else:
                  return line
        except EOFError: # Should not happen if recvuntil handles EOF correctly
             # This might be reachable if context.throw_eof_on_incomplete_line is True
             # and recvuntil re-raises EOF.
             self.log("EOFError propagated from recvuntil in recvline.", "warning")
             raise
...
class tube_base(metaclass=abc.ABCMeta):
    """
    Base class for all tubes (sockets, processes, etc.), inspired by pwntools tube.
    Provides high-level API and manages buffering and timeouts.
    Relies on subclasses implementing `*_raw` methods.
    """

    def __init__(self, timeout = "default", level = None, buffer_fill_size = 4096):
        # Timeout Handling
        self._timeout = None # Internal storage
        self.settimeout(timeout) # Use setter to handle "default"

        # Buffering
        self._buffer = b""
        self.buffer_fill_size = buffer_fill_size # How much to request from recv_raw

        # Logging Level (optional direct setting)
        self._log_level = level # Store requested level

        # Ensure subclass implements close even if not abstract (for atexit)
        # atexit.register(self.close) # Consider if atexit is desired
    ...
    def timeout(self, value):
        if value == "default":
            self._timeout = context.get_timeout()
        elif value is not None and not isinstance(value, (int, float)):
            raise ValueError("Timeout must be None, 'default', or a number")
        elif value is not None and value < 0:
            raise ValueError("Timeout cannot be negative")
        else:
            self._timeout = value
        # Notify the raw layer implementation of the change
        self.timeout_change()
    ...
    def timeout_change(self):
        """Callback method for subclasses when timeout changes."""
        # Call the raw implementation if it exists
        try:
            self.settimeout_raw(self._timeout)
        except NotImplementedError:
            pass # Subclass might not need explicit timeout setting
        except Exception as e:
            # Log potential issues setting timeout on the raw layer
            log(f"Ignoring error during timeout_change -> settimeout_raw: {e}", "debug")
